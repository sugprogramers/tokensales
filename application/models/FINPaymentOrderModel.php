...
class FINPaymentOrderModel extends CI_Model
{
    ...
    public function get_project_investments_ipayout($cProjectManagerId){
        $this->db->select("po.fin_payment_order_id, p.name as projectName, p.name as companyName, i.c_investor_id, u.email as investorEmail, (u.firstname || ' ' || u.lastname) as investorname, curr.iso_code, po.amount, po.scheduleddate");
        $this->db->from('fin_payment_order as po');
    ...
    public function get_projectInvestmentInfoByOrderPaymentId($finPaymentOrderId){
        ...
        $this->db->select("p.name as projectName, p.name as companyName, i.c_investor_id, u.email as investorEmail, (u.firstname || ' ' || u.lastname) as investorname");
        $this->db->from('fin_payment_order as po');
        $this->db->join('fin_investment  as iv', 'po.fin_investment_id = iv.fin_investment_id ');
        $this->db->join('c_project  as p', 'iv.c_project_id = p.c_project_id ');
        $this->db->join('c_investor as i', 'iv.c_investor_id = i.c_investor_id ');
    ...
    public function get_payOutPaymentOrderByInvestorId($cInvestorId){              
